...
- QA spiegelt den Hochstufen-Check mit `tools/test_chronopolis_high_tier.js`: Die Simulation setzt eine Chief-Agentin mit Research 4 an, prüft, dass alle Slots ohne 🔒-Locks erscheinen, und dass Px-Klauseln/Notizen per `log_market_purchase()` im Debrief (`Chronopolis-Trace`) landen. Der QA-Fahrplan dokumentiert den Lauf unter QA-Follow-up #14.
