...
Nächste Schritte
- Maintainer:innen: Entscheidung treffen: Backlash behalten (mit kanonischem Toast) oder Px-Verlust als Mechanik streichen. Ergebnis in `ZEITRISS-qa-audit-2025.md` dokumentieren.
...
Nächste Schritte
- Maintainer:innen: Exploding-W10-Entscheidung treffen (ja/nein) und in Regelkern spiegeln. Kein Breaking Change, aber Klarheit für Spielleitung und Spielende notwendig.
...
Nächste Schritte
- Maintainer:innen: Entscheiden, ob Level-Up-Optionen nummeriert (1/2/3) oder als Freitext angeboten werden. Nummeriert erhöht Fehleranfälligkeit bei Doppelwahl; Freitext erfordert mehr Spielleiter-Interpretation.
...
Nächste Schritte
- Maintainer:innen: Flag-Migration für bestehende Saves definieren (default `false` bei fehlendem Flag). `chronopolis_unlock`-Trace-Event in `logs.trace[]` standardisieren.
...
Nächste Schritte
- Maintainer:innen: Prüfen, ob das 3-Absatz-Minimum in allen Phasen (Briefing, Infil, Debrief) gleich gilt oder nur in Action-Szenen. Ggf. phasenspezifische Minima definieren.
...
Lösungsvorschlag
- Ansatz: Load-Flow um Cross-Mode-Erkennung erweitern: Wenn `team.members[]` leer und Ziel-Mode ist Koop, automatisch `merge_conflicts.mode_switch` schreiben und `ui_host_override`-Trace erzeugen. Toast: `Kodex: Solo-Save in Koop-Modus geladen. Team-Slots leer — bitte Mitspieler:innen hinzufügen.`
...
Nächste Schritte
- Maintainer:innen: Cross-Mode-Matrix dokumentieren (welche Felder überschrieben vs. behalten werden). In QA-Audit aufnehmen.
...
Nächste Schritte
- Maintainer:innen: Entscheiden, ob "temporal"-Budget auf 5× erhöht wird (realistischer) oder bei 3× bleibt (strenger Stilkodex). Aktuelle Grenze erscheint bei langen Rift-Szenen zu eng.
...
Nächste Schritte
- Maintainer:innen: Fixture `mission5_badge_snapshots.json` um Abbruch-Snapshot erweitern. `test_acceptance_followups.js` um Abbruch-Testpfad ergänzen.
...
Nächste Schritte
- Maintainer:innen: HUD-Goldenfile in `dispatcher_strings` um Rift-Variante erweitern.
...
Nächste Schritte
- Maintainer:innen: Wallet-Key-Kanonisierung als separates Schema-Update versionieren. Fixture `savegame_v6_test.json` mit korrekten Wallet-Keys aktualisieren.
...
*Report generiert: 2026-03-01 00:58 CET*
