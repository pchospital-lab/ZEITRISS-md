...
| ZR-021 | Post-139 Onboarding-Hardening fortführen | `uploads/ZEITRISS_start_mmo_onboarding_review.md` | P1 | Runtime/QA + Meta/Prozess | abgeschlossen | Hard-Final-Review-Anschlussdurchläufe 140–178 erledigt; Durchlauf 176 konsolidiert den `iti-hardcanon-watchguard` vollständig auf den zentralen Watchguard-Doc-Loader (ohne lokalen Resolver-Resthelfer), Durchlauf 177 ergänzt den Meta-Watchguard `watchguard-loader-consistency` als Smoke-Pflichtanker gegen Resolver-Rückfälle, Durchlauf 178 härtet den `director-layer-watchguard` für die Weltstatus-Rückkopplung inkl. verpflichtendem Folgewirkungsanker (inkl. `gameplay/kampagnenstruktur.md`). Operativer Folgestand in `internal/qa/process/hard-final-review-next-steps.md`. |
