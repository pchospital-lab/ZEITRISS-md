...
| ZR-021 | Post-139 Onboarding-Hardening (Entry-Layer-Guard + optionales Archetypen-Modul) fortführen | `uploads/ZEITRISS_start_mmo_onboarding_review.md` | P1 | Runtime/QA + Meta/Prozess | in Umsetzung | Fokus für Anschlussläufe: (1) Archetypen/Pregens bleiben außerhalb des Default-Wissensspeichers, (2) README/Setup-Guide/Setup-Script werden per Smoke-Watchguard gegen Onboarding-Drift abgesichert, (3) Prozessspur über Plan/Log/known-issues fortlaufend nachziehen. Stand: Durchläufe 140–141 umgesetzt. Durchlauf 142 priorisiert zusätzlich den spieler-sichtigen Mini-Startpfad (`solo klassisch` als Standard, `solo schnell` als optionale Fast-Lane) und härtet diese Priorisierung per Onboarding-Watchguard (`internal/qa/plans/issue-pack-durchlauf-142-spielerhandbuch-startpfad-priorisierung-watchguard.md`, `internal/qa/logs/2026-03-10-issue-pack-durchlauf-142-spielerhandbuch-startpfad-priorisierung-watchguard.md`). Durchlauf 143 zieht diese Hierarchie jetzt auch in den Start-Transkripten nach (klassischer Kampagnenstart als Standardbeispiel, Schnelleinstieg explizit als Fast-Lane optional) und erweitert den Watchguard um entsprechende Transcript-Assertions (`internal/qa/plans/issue-pack-durchlauf-143-start-transkripte-standard-vs-fastlane-hardening.md`, `internal/qa/logs/2026-03-10-issue-pack-durchlauf-143-start-transkripte-standard-vs-fastlane-hardening.md`). |
## Pflegehinweis
