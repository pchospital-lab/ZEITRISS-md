...
- **Pflichtformat `shared_echoes[]` / `roster_echoes[]`:** Jedes Item MUSS Objekt mit mindestens `tag` sein, empfohlen vollständig `{ "tag": "<slug>", "scope": "shared|rumor|campaign|personal", "text": "<kurz>" }`. Rohstrings (`["Lagerhaus gesichert"]`) oder Fremdkeys (`[{"echo": "..."}]`) sind Schema-Verletzung und werden von `test_v7_schema_consistency.js` + `test_continuity_output_contract.js` abgelehnt. Siehe Masterprompt §D `shared_echoes`-Pflichtformat.
