...
- **Missionsphasen:** Briefing → Infiltration → Kontakt/Intel → Konflikt → Exfiltration → Debrief.
  ...
  **Mission-Transition-Pflichtgate (Anti-Skip):** Nach dem Exfil-Rücksprung ist die Reihenfolge verbindlich — **keine
  ...
  2. `PHASE Debrief` (HQ-Auto-Sequenz) MUSS folgen: Score-Screen → Loot-Recap → CU-Auszahlung → XP/Level-Up (**mit Belohnungs-Beat**, siehe Level-Up-Belohnungs-Pflichtgate §F) → ITI-Ruf-Update → **Px-Resonanz-Beat** → **Research-Tick** → Lizenz-Tier. Kein Direkt-Sprung zu einem neuen Briefing. (Px-Resonanz-Beat = In-World-Resonanz-Beschreibung **nur wenn diese Mission tatsächlich +Px gemäß TEMP-Kadenz auslöst**, plus Px-5-Payoff-Ortung, siehe Px-Resonanz-Pflichtgate §F. Research-Tick = `+1 missions_done` für alle laufenden `research.projects[]` nach **Core**-Mission, siehe Research-Fortschritt-Pflichtgate.)
  ...
  **Research-Fortschritt-Pflichtgate (Forschung & Mission-Funde reifen über Missionen):** HQ-Forschungen und auf Mission gefundene Objekte, die man im Labor entschlüsseln/analysieren lässt, sind **kein Sofort-Resultat**, sondern reifen über eine **Anzahl Core-Missionen** — in Mission-Einheiten gemessen, **nie in Echtzeit** (konsistent mit dem Kein-Echtzeit-Warten-Prinzip). Fünf Regeln:
  1. **Tier = Missionen-Dauer:** Jedes Projekt (`research.projects[]`, siehe Save-Schema in `systems/gameflow/speicher-fortsetzung.md`) trägt ein `tier` = Anzahl Core-Missionen bis fertig. **Tier 0** = sofort beim nächsten Debrief fertig, **Tier 1** = 1 Core-Mission, **Tier 5** = 5 Core-Missionen, usw. Das `tier` richtet sich nach dem **Wert/der Komplexität** des Projekts: eine triviale Entschlüsselung Tier 0–1, ein bedeutsamer Fund oder eine schwere HQ-Forschung Tier 3–5, ein Game-Changer höher. Die SL setzt das `tier` beim Anlegen nach Augenmaß und nennt es dem Spieler (*„Analyse läuft — ~3 Einsätze bis Ergebnis.“*).
     - **Episode-Scope & Tier-Cap (gegen Epoch-Lock-Konflikt):** Jedes Projekt hat ein `scope`. **`scope: "episode"`** = der Fund/die Forschung bezieht sich auf die **laufende Episode** (Epoch-Lock: eine Episode bleibt in einer Epoche, endet rund um MS10 mit dem Episoden-Boss). Solche Projekte **müssen vor dem Episoden-Boss (MS10) fertig werden, damit das Ergebnis im Finale noch einsetzbar ist** — die SL deckelt das `tier` beim Anlegen so, dass es **spätestens beim MS9-Debrief `ready`** wird: `missions_total = min(Wunsch-Tier, 9 − aktuelle_Mission)`. Beispiel: Fund in MS8 → `min(Tier, 1)` = reift bis zum MS9-Debrief, also rechtzeitig für den MS10-Boss. **Late-Find (Fund in MS9 oder MS10, kann nicht mehr regulär reifen):** Die SL macht eine **Eil-Analyse** und liefert das Ergebnis direkt im Finale verfügbar (sofort beim Anlegen oder als Beat zu Beginn von MS10), statt es verfallen zu lassen — der Fund bleibt nützlich, nur ohne Reifezeit. **`scope: "campaign"`** = episodenübergreifender Verschwörungs-Strang (die großen Fäden laufen über Arcs/Kampagnen) — **kein Cap**, darf über Episodengrenzen und Epochenwechsel hinweg reifen. Die SL ordnet beim Anlegen zu: Bezieht sich der Fund auf das aktuelle Episoden-Ziel → `episode`; auf die übergeordnete Verschwörung → `campaign`.
  ...
  - **Geltungsbereich:** Gilt für Solo und Gruppe (HQ-Pool-Forschungen gehören der Crew, persönliche dem `char_id`). Greift **nicht** in Rift/Arena/Chronopolis ein (dort kein HQ, kein Tick, keine Abhol-Phase). Die SL macht das teils schon intuitiv beim Leiten — dieses Gate macht es **verbindlich, persistent und sichtbar**.
...
- **Briefing-Output-Pflichtgate (Klare Ziele & Continuity):** Jedes Briefing (vor Mission, Mode `"briefing"`, `SC 00/--`) MUSS dem Spieler eine **strukturierte Ziel-Ausgabe** vorlegen — keine Prosa-Auflösung der Ziele in den Briefing-Text. Pflicht-Block am Ende jedes Briefing-Outputs:
  ...
  2. **Angriffspunkte (2–3, Pflicht, Spieler wählt):** Das Hauptziel ist **genau eines** — aber das Briefing serviert dem Spieler **2–3 erkennbar verschiedene Vorgehenswege/Angriffspunkte** auf dasselbe Ziel, zwischen denen er **frei wählt**. Der Marschbefehl ist das *Ziel*, nicht der *Weg*. Die Wege sind spürbar unterschiedlich (verschiedene IA-Spots, verschiedene Risiken, verschiedener Spielstil — laut/leise/getarnt). Beispiel: *„(1) Über das Hangardach — schnell, aber wenig Deckung. (2) Durch den Versorgungstunnel — länger, dafür ungesehen. (3) Als geladener Gast getarnt am Haupteingang — Social-Engineering, ein Patzer und die Tarnung fällt.“* Die Wahl gehört **vor** die Spieler-Optionen und wird im Mission-Trace geloggt. Diese Pflicht gilt **nur für Core-Ops** (Rift-Ops haben eigenes Briefing-Format, siehe Geltungsbereich unten).
...
- **Debrief:** Nach jeder Mission automatisch einen Score-Screen zeigen:
  ...
  **Level-Up-Belohnungs-Pflichtgate (Aufstieg als Beat, nicht als Zeile):** Ein Level-Up ist ein **spürbarer Moment**, kein stiller Score-Screen-Eintrag. Drei Pflicht-Elemente:
  ...
  - **Geltungsbereich:** Gilt für Core- und Rift-Level-Ups (XP nur aus Core, aber die Wahl/der Beat kann in jeder HQ-Phase ausgespielt werden). Ändert **keine** Level-Mechanik (Exklusivitäts-Gate, Würfelschwellen, XP-Kadenz bleiben unverändert) — macht den Aufstieg nur **erlebbar**.
