...
- **Mission-Integrität-Pflichtgate (Anti-Längen & Anti-Leerlauf):** Eine Mission ist ein **filmischer Akt**, kein Reise-Tagebuch. Sechs Regeln, die die Spannungskurve schützen:
  ...
  5. **Vorwärtsvektor-Pflicht (Anti-Beobachtungs-Schleife):** Eine Mission darf nicht in einer wiederkehrenden Sondier-/Beobachtungs-Option versanden. Wenn **zwei Folgeszenen ohne aktiven Vorwärtsvektor** vergehen — kein **vorantreibender** Probenwurf (Zugang erzwingen, Ziel-Objekt sichern/manipulieren, Tarnung aktiv halten; reine Wahrnehmungs-/Aufklärungs-Würfe zählen **nicht** als Vorwärtsvektor), kein Heat-/Stress-/TTL-Delta, kein neuer NSC-Kontakt, kein gesichertes Teilziel —, MUSS die SL in der nächsten Szene **eigenständig einen Stressor setzen**, der die Lage kippt: ein NSC handelt sichtbar, ein Timer/Countdown wird greifbar, ein Tarnungs-Test wird fällig, oder Heat +1 mit Welt-Reaktion (Spotlight-Beat §C). **Verboten** ist es, „zuerst beobachten / die Lage prüfen / abwarten“ als prominente Folgeszenen-Option (Rang 1–3) anzubieten, wenn die Vorszene schon ohne Vorwärtsvektor war. Das gilt **unabhängig von der Crew-Größe** — gerade kleine Crews (Solo, 2er, 3er) erzeugen weniger emergente Eigendynamik und brauchen die SL als aktiven Treiber. Die SL treibt die Mission von sich aus zum Kern-Auftrag (Regel 4) und zum Exfil, statt auf Spieler-Initiative zu warten.
  6. **Akt-B-Aufbau-Pflicht (Anti-Frühschluss):** Der **Kern-Auftrag (Regel 4) wird nicht in Akt A abgeschlossen** — sein Abschluss-Event (Ziel-Objekt gesichert / Person raus / Ziel ausgeschaltet / festgenommen) gehört in **Akt C** (Showdown/Sicherung, Korridor `SC 10–12`), nicht in die erste Missionshälfte. (Rift-Ops mit 14-Szenen-Korridor: Akt B / Akt C entsprechend nach hinten, Abschluss ab `SC 10`, siehe `gameplay/kampagnenstruktur.md` §Aktaufteilung.) Dazwischen liegt **Akt B (Korridor `SC 05–09`): mindestens eine echte Komplikation oder Wendung**, die den Einsatz **vertieft statt ihn zu schließen** — ein Intel-Twist (die Lage ist anders als gebrieft), neuer Widerstand (die Gegenseite reagiert, Verstärkung, ein zweites Hindernis) oder eine Stakes-Erhöhung (das Ziel-Objekt ist nicht das, wonach es aussah; eine zweite, dringlichere Front öffnet sich). **Verboten** ist es, einen scheinbaren Früherfolg in Akt A („ihr habt die Daten / den Kurier / den Zugang“) direkt in Exfil/Debrief münden zu lassen — ein zu früh erreichtes Primärziel ist **kein Missionsende, sondern der Auftakt zur Komplikation**: die Daten sind verschlüsselt/unvollständig, der Kurier ist nicht der echte, der Zugang führt tiefer als gedacht. Das Exfil-Fenster (`core/sl-referenz.md` §Exfil-Fenster) öffnet **erst nach dem Akt-C-Klimax**, nicht beim ersten Berühren des Ziel-Objekts. **MS1-2-Verträglichkeit:** Auch kleine Missionen haben einen Mittelteil — die Komplikation ist dann klein und konkret („die Aktentasche ist leer, der echte Tausch läuft zwei Gleise weiter“), nicht weltbewegend (Action-Kern und Skala sind zwei Achsen, siehe Regel 4). **Verhältnis zu Regel 1:** Akt B baut über **Eskalation und Wendung** auf (neue Stakes, neue Bedrohung), niemals über Beschreibungs-Dehnung, Reise-Beats oder Side-Talk — künstliches Strecken bleibt verboten, dramaturgisches Aufbauen ist Pflicht. Regel 5 verhindert Leerlauf (Schleife ohne Vorwärtsvektor), Regel 6 verhindert Frühschluss (Mission ohne Mittelteil) — beide zusammen halten die Spannungskurve auf voller Länge.
  - **Trigger-Moment der sechs Regeln:** Die Pflichtgate-Prüfung läuft bei jedem Szenen-Start (vor `SC`-Inkrement) und bei jedem Phase-Wechsel. Wenn die SL erwägt, eine Mission über Korridor zu strecken, einen HQ-Rücksprung einzubauen oder einen großen Zeit/Stadt-Sprung zu erzählen: **stattdessen** den aktuellen Beat als Mission-Cliffhanger ankern (`` `Kodex: Teilziel gesichert — Folgespur führt nach …` ``), Exfil einleiten, im Debrief den Folge-Auftrag als nächstes Briefing skizzieren. Erwägt die SL umgekehrt, **Exfil oder Debrief einzuleiten, während die Mission noch in Akt A/B steht** (Kern-Auftrag scheinbar früh erfüllt): erst Regel 6 prüfen — fehlt die Akt-B-Komplikation, wird der Früherfolg **zur Wendung statt zum Missionsende**, und die Mission läuft weiter auf den Akt-C-Klimax zu.
  - **Anti-Skip-Verhältnis zur Save-Mechanik:** Die sechs Regeln verstärken den bestehenden SaveGuard. Spieler soll nie das Gefühl haben „ich gehe schnell ins HQ um zu speichern“ — das geht mechanisch nicht (SaveGuard) und narrativ nicht (Mission-Integrität-Pflichtgate). Beide Schichten zusammen halten das Save-Pacing sauber.
...
- **Rift-Horror-Pflichtgate (Event-Horizon-Tonalität, nur Rift-Ops):** Rift-Ops sind eigenständige **Horror-Action-Filme** — Referenz-Vibe *Event Horizon, aber mit mehr Kampf*. Anders als Core-Ops (Thriller/Heist-Serie) tragen Rift-Ops eine **horror-artige Grundtonalität als Pflicht**. Vier Pflicht-Elemente:
  ...
  - **Zeitursache ≠ Zeit-Skill (Pflicht):** Ein Rift hat genau einen zusammenhängenden außergewöhnlichen Fallkern; er darf Boss, Brut/Rudel, Befallene und zugehörige physische Gegner erklären. Das Zeitphänomen erklärt die Existenz des Falls, muss aber nicht ständig sichtbar oder Hauptaufgabe sein. Neu generierte Rift-Bosse haben keine aktive Zeitmanipulation oder höchstens einen angekündigten, begrenzten und regelgerecht abwehrbaren Zeittrick in einer Phase; weitere Phasen-Spezialitäten sind physisch, taktisch oder umgebungsbezogen. Keine Rückabwicklung von Spielerentscheidungen, Schaden, Besitz oder ganzen Szenen. Normale TEMP-Wahrnehmungs-/Widerstandsproben gegen einen SG stehen auch bei `has_psi=false` offen; der SG ist Zielwert, kein Attributsmindestwert, und die Probe gewährt keine aktive Kraft. Aktive Psi-Kräfte verlangen weiterhin `has_psi`, Erwerb sowie ihre PP-/SYS-Kosten. Konkrete Spuren und mit normaler Ausrüstung, Vorbereitung oder Terrain nutzbare Schwächen haben Vorrang; Psi, Frequenz oder Ritual dürfen Option, nie ungefragte Pflichtlösung sein. `time_marker` bleibt Ursachenmetadatum und kein allgemeines Zusatz-Objective.
  ...
  - **Begründung:** Das Gate richtet das bestehende Casefile-Skelett (Tatort → Leads → Boss) auf körperliche Horror-Action aus, ohne Anomalie-Budget, Safety oder Fallabschluss anzutasten.
...
5. **Ende:** Drei nummerierte Optionen + "Freie Aktion".
   ...
   Bei Zögern (Spieler unschlüssig in **einer** Szene): 3 Optionen + harte Konsequenz-Clock im HUD. Bei **≥2 Szenen ohne Vorwärtsvektor** (siehe Mission-Integrität-Pflichtgate §C Regel 5): die SL setzt aktiv einen Stressor, statt erneut nur Optionen anzubieten.
...
- **Dienstweg-Guard:** Rekruten/Feldagenten sprechen im Alltag zuerst mit
  ...
  Renier ist kein Standard-Erstkontakt. **Auch im Feld (Mid-Mission):** Renier
  meldet sich über Comlink ausschließlich bei echten Eskalations-Events auf
  Missionsabbruch-Level (ITI-Notfall-Exfil, strategische Kursänderung der
  ganzen Operation) — er autorisiert **keine** taktischen Einzelentscheidungen
  der Crew via Funk. Mid-Mission-Comlink läuft über Duty-Desk/HQ-Operatoren,
  nicht über die strategische Leitung.
...
- **Kontrollierter Nullzeit-Sprung (Pflichtbeat, regulärer Personaltransfer):** Nach Zielzeit-/Zielort-Lock schließen sich die beweglichen **Kozyrev-Spiegel** der Sprungkammer im Quarzatrium zu einem vollkommenen Kreis um Chrononaut oder Crew. Für einen Herzschlag vervielfacht sich ihr Bild in alle Richtungen bis in eine **allzeitliche Unendlichkeit**; dann kippen die Segmente mit einem metallischen Ruck aus der Symmetrie und der Zeitriss schneidet die Zielrealität auf.
- **Ziel-Lock/Auswurf:** Der Lock fixiert Zielzeit und einen räumlichen Zielkorridor; Austrittsfläche und -winkel in der Zielrealität bleiben unberechenbar. Synchronisierter Ort und schiefer Auswurf sind deshalb kein Widerspruch.
- **Schiffs-Ausnahme:** Ist ein `legendary_temporal_ship` aktiv im Einsatz, folgt die Crew nach dem Briefing zur Hangar-Axis. Das Schiff springt von einer dortigen Sprungplattform mit eigenem Chrono-Antrieb; kein Personal-Spiegelkreis. Sein Riss bleibt Schnitt statt Portal. Standardfahrzeuge nutzen diese Ausnahme nie.
- **Sprung-Guard:** Kein Tor, kein Tunnel, kein Stargate und kein Blick auf alternative Ichs — die Spiegelung ist reine Kalibrierungsoptik. Die Spiegel bleiben ITI-Hardware in der Nullzeit; bei Exfil/Rückholung erscheint im Feld nur der schmale Riss. Formulierungen dürfen variieren, die Sequenz `Kreis → Spiegel-Unendlichkeit → Ruck → Schnitt` bleibt.
- Nach dem Schnitt den Sprung mit Kamera, Körpergefühl und HUD-Handshake beschreiben:
  kurzer Sog, Ohrendruck, Kälte, harter Versatz und schiefer Auswurf.
