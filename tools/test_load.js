...
const hostSeeds = Array.from({ length: 2 }, (_, idx) => ({ id: `R-H-${idx + 1}` }));
const incomingSeeds = Array.from({ length: 12 }, (_, idx) => ({ id: `R-I-${idx + 1}` }));
rt.state.location = 'HQ';
rt.state.campaign = { rift_seeds: hostSeeds, mode: 'preserve', seed_source: 'preserve' };
rt.state.character = { id: 'HOST', stress: 0, psi_heat: 0, cooldowns: {}, attributes: { SYS_max: 1, SYS_used: 1 } };
rt.state.team = { members: [] }; rt.state.party = { characters: [] }; rt.state.loadout = {};
rt.state.economy = {}; rt.state.arc_dashboard = { offene_seeds: [], fraktionen: {}, fragen: [] };
rt.state.logs = { flags: {} };
const mergeSave = { save_version: 6, location: 'HQ', phase: 'core',
  campaign: { rift_seeds: incomingSeeds, px: 0, paradoxon_index: 0 },
  character: { id: 'IMPORT', stress: 0, psi_heat: 0, cooldowns: {}, attributes: { SYS_max: 1, SYS_used: 1 } },
  team: { members: [] }, party: { characters: [] }, loadout: {}, economy: {},
  logs: { flags: {} }, ui: { gm_style: 'verbose' }, arc_dashboard: { offene_seeds: [] } };
