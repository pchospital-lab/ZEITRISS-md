...
// 3) chrono_launch_rift: nur HQ; kein Episodengate
must(/macro\s+chrono_can_launch_rift\(\)[\s\S]*campaign\.loc\s*==\s*'HQ'/, 'chrono_can_launch_rift()-HQ-Gate fehlt.');
must(/macro\s+chrono_launch_rift\(seed_id\)[\s\S]*chrono_can_launch_rift\(\)\s*!=\s*'true'[\s\S]*Rift-Start blockiert\s*-\s*nur im freien HQ/, 'chrono_launch_rift()-Blockzweig fehlt oder driftet.');
